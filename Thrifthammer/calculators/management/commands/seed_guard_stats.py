...
for all Astra Militarum units in the database. Safe to re-run — uses
...
GUARD_UNITS = [
    ...
    ('Astra Militarum Commissar', 30,
     ...
     [
         ability('Leader',
                 ...
                 'Death Korps of Krieg, Kasrkin, Krieg Combat Engineers, '
                 'Tempestus Scions'),
         ability('Summary Execution',
                 'Once per battle round, at the start of any phase, you can '
                 'select one friendly Astra Militarum Infantry unit within 3" '
                 'of this model. Until the end of the phase, that unit is not '
                 'affected by the Battle-shock or Leadership rules. Then, roll '
                 'one D6: on a 1, one model in that unit (your choice) is slain.'),
     ]),
    ...
    ('Astra Militarum Basilisk', 140,
     stat('10"', 9, '3+', 11, '7+', 3),
     [
         rng('Earthshaker cannon', '240"', 'D6+3', '4+', 8, -2, 2,
             'Blast, Indirect Fire'),
         rng('Heavy bolter',       '36"',  '3',    '4+', 5, -1, 2,
             'Sustained Hits 1'),
         mel('Armoured tracks',    '3',    '4+',   6, 0, 1),
     ],
     [
         ability('Earthshaker Rounds',
                 'In your Shooting phase, after this model has shot, if one '
                 'or more of those attacks made with its earthshaker cannon '
                 'scored a hit against an enemy Infantry unit, until the start '
                 'of your next Shooting phase, that unit is shaken. While a '
                 'unit is shaken, subtract 2" from its Move characteristic and '
    ...
    ('Astra Militarum Chimera', 85,
     ...
     [
         ability('Mobile Command Vehicle',
                 'In your Command phase, one Officer model embarked within '
                 'this Transport can issue Orders even though it is not on '
                 'the battlefield. When doing so, measure distances to and '
                 'from this Transport.'),
     ]),
    ...
    ('Astra Militarum Hellhound', 125,
     stat('10"', 10, '2+', 11, '7+', 3),
     [
         rng('Inferno cannon', '18"', '2D6', 'N/A', 6, -2, 1,
     ...
     [
         ability('Flush Them Out',
                 'In your Shooting phase, after this model has shot, select '
                 'one enemy unit that was hit by one or more of those attacks. '
                 'Until the start of your next Shooting phase, that unit is '
                 'scattered. While a unit is scattered, it cannot have the '
                 'Benefit of Cover.'),
         ability('Damaged: 1-4 Wounds Remaining',
                 'While this model has 1-4 wounds remaining, each time this '
                 'model makes an attack, subtract 1 from the Hit roll.'),
    ...
    ('Astra Militarum Leman Russ Battle Tank', 185,
     stat('10"', 11, '2+', 13, '7+', 3),
     [
         rng('Leman Russ battle cannon', '48"', 'D6+3', '4+', 10, -1, 3,
             'Blast'),
         rng('Lascannon',               '48"', '1',     '4+', 12, -3, 'D6+1'),
         mel('Armoured tracks',         '6',   '4+',    7, 0, 1),
     ],
     [
         ability('Armoured Spearhead',
                 'Each time this model makes an attack that targets an enemy '
                 'unit, re-roll a Hit roll of 1 and, if that unit is within '
                 'range of an objective marker you do not control, you can '
    ...
    ('Astra Militarum Sentinel', 55,
     ...
     [
         ability('Daring Recon',
                 'At the start of your Shooting phase, select one enemy unit '
                 'within 18" of and visible to this unit. Until the end of '
                 'the phase, each time a friendly Astra Militarum model makes '
                 'an attack that targets that unit, re-roll a Hit roll of 1.'),
     ]),
