"""
Management command: review_monday_custom_deals

Read-only reporting tool -- NOT part of the real Monday send and never
touches real subscribers. Loops through every confirmed monday_custom
...
subscribers would see it and which category it belongs to.
...
(python manage.py send_monday_custom_deals).

Reuses send_monday_custom_deals.Command._get_category_deals() by importing
...
Usage:
    python manage.py review_monday_custom_deals
    python manage.py review_monday_custom_deals --limit 15
"""
...
from products.management.commands.send_monday_custom_deals import (
    Command as MondayCustomDealsCommand,
)
...
class Command(BaseCommand):
    """Email the admin a deduplicated union of every Monday subscriber's deals."""

    help = (
        "Read-only: email the admin a deduplicated union of every unique "
        "deal that would appear in ANY confirmed Monday-custom subscriber's "
    ...
    def handle(self, *args, **options):
        ...
        if not subscribers:
            self.stdout.write(self.style.WARNING(
                'No confirmed Monday custom subscribers -- nothing to review.'
            ))
            return

        self.stdout.write(f'{len(subscribers)} confirmed Monday subscriber(s) to check.\n')
        ...
        deal_source = MondayCustomDealsCommand()
        ...
        for sub in subscribers:
            ...
            deals = deal_source._get_category_deals(category_list, limit)
            if not deals:
                skipped_no_deals += 1
                continue
        ...
        self.stdout.write(f'{len(deals_list)} unique deal(s) across all Monday subscribers:')
        ...
        subject = (
            f"[ADMIN REVIEW] Monday Custom Digest -- {len(deals_list)} Unique Deals "
    ...
    def _build_text_body(self, deals, today, subscriber_count):
        ...
        lines = [
            'THRIFTHAMMER -- ADMIN REVIEW: MONDAY CUSTOM DIGEST',
        ...
        lines += [
            ...
            'with: python manage.py send_monday_custom_deals',
        ]
        return '\n'.join(lines)
