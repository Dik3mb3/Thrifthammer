"""
Management command: update_gw_uk_prices

Re-fetches the current price from each tracked product's own Games
Workshop UK product page and updates both the games-workshop-uk
CurrentPrice and Product.msrp_gbp when the price has changed.

UK counterpart of update_gw_prices -- same site (warhammer.com), same
AWS WAF Bot Control, same fix. See that command's docstring for the full
rationale; the short version repeated here:
...
Usage:
    python manage.py update_gw_uk_prices                    # all tracked GW UK products
    python manage.py update_gw_uk_prices --dry-run           # preview only
    python manage.py update_gw_uk_prices --sku 48-75         # single product
    python manage.py update_gw_uk_prices --skus "48-75,53-23"
    python manage.py update_gw_uk_prices --batch-tag skaven
    python manage.py update_gw_uk_prices --category "Warhammer 40,000"
    python manage.py update_gw_uk_prices --limit 50
    python manage.py update_gw_uk_prices --delay 3.0
    python manage.py update_gw_uk_prices --headless          # not recommended -- more likely to be challenged
"""
...
class Command(BaseCommand):
    """Re-check GW UK product page prices; update CurrentPrice + Product.msrp_gbp on change."""

    help = 'Re-check GW UK product page prices and update CurrentPrice + Product.msrp_gbp when changed. Run locally only -- see docstring.'
    ...
    def handle(self, *args, **options):
        """Main entry point -- fetch each tracked GW UK page and reconcile the price."""
        try:
        ...
        self.stdout.write('\nGW UK Price Check (Playwright, local run)')
    ...
    def _save_price(self, cp, product, new_price):
        """Persist a changed price to both CurrentPrice and Product.msrp_gbp."""
        cp.price = new_price
    ...
    def _extract_price(self, html):
        ...
        proven in import_gw_catalog.py's one-time scraper and in
        update_gw_prices.py for the US site. The JSON-LD "price" value is
        a bare number with no currency symbol either way, so no GBP-vs-USD
        handling is needed here -- the currency is implied by which site
        (en-GB) the URL points at, which the caller already filtered on.
        """
